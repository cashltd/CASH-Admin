...
{literal}
	...
	function freshchange(type) {
		...
		claimp.stop();
		claimp.url = '{/literal}{$HomeUrl}{literal}ajax/claimants_freshdata/id/'+provider+'-'+freshpagestart+'-'+perpage+'-'+userid+'/';
		claimp.start();
	...
	function declinechange(type) {
		...
		claimr.stop();
		claimr.url = '{/literal}{$HomeUrl}{literal}ajax/claimants_olddata/id/'+provider+'-'+declinepagestart+'-'+perpage+'-'+userid+'/';
		claimr.start();
	...
	function acceptchange(type) {
		...
		claimq.stop();
		claimq.url = '{/literal}{$HomeUrl}{literal}ajax/claimants_newdata/id/'+provider+'-'+acceptpagestart+'-'+perpage+'-'+userid+'/';
		claimq.start();
...
{literal} 
// <![CDATA[
var claimp = new Ajax.PeriodicalUpdater('toVerify', '{/literal}{$HomeUrl}{literal}ajax/claimants_freshdata/id/'+provider+'-'+freshpagestart+'-'+perpage+'-'+userid+'/', {
...
{literal} 
// <![CDATA[
var claimq = new Ajax.PeriodicalUpdater('claimQueue', '{/literal}{$HomeUrl}{literal}ajax/claimants_newdata/id/'+provider+'-'+acceptpagestart+'-'+perpage+'-'+userid+'/', {
  method: 'get', frequency: 30});
// ]]>
{/literal} 
</script>
...
{literal} 
// <![CDATA[
var claimr = new Ajax.PeriodicalUpdater('noVerify', '{/literal}{$HomeUrl}{literal}ajax/claimants_olddata/id/'+provider+'-'+declinepagestart+'-'+perpage+'-'+userid+'/', {
  method: 'get', frequency: 30});
// ]]>
{/literal} 
</script>
