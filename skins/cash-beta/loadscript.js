...
// CONVERTED jQUERY FUNCTIONS ARE BELOW!
	...
	// Functions related to the Claimants section (Information that comes from a CSV)
		...
		function claimant_callback(id, title) { // Mark a claimant as a callback 
		    ...
		    var $dialog = $j('<div></div>')
		    .html('<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Are you sure you want to set ' + title + ' as a callback?</p>')
		    .dialog({
		        autoOpen: false,
		        title: cltitle,
		        resizable: false,
		        ...
		        buttons: {
		            "YES!": function() {
		                $j(this).load('http://' + document.domain + '/ajax/claimants_goCallback/id/' + id + '/', null,
		                function(responseText) {
